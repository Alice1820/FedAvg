...
class TSN(nn.Module):
    def __init__(self, name='TSN', num_class=120, num_segments=8, modal='RGB',
    ...
    TSN Configurations:
        ...
        if self.modality == 'Flow' or self.modality == 'AB':
            ...
            print("Converting the ImageNet model to a L init model")
            self.base_model = self._construct_l_model(self.base_model)
            print("Done. L model ready...")
    ...
    def _construct_l_model(self, base_model):
        ...
        print('#' * 30, 'Warning! No L pretrained model is found')
        return base_model
