...
class TSMV1(nn.Module):
    def __init__(self, name='resnet50', args=None):
        super(TSMV1, self).__init__()
        if name == 'resnet50':
            # self.l_to_ab = TSN(num_segments=args.num_segments, num_class=args.feat_dim)
            # self.ab_to_l = TSN(num_segments=args.num_segments, num_class=args.feat_dim)
            self.l_to_ab = TSN(num_segments=args.num_segments, num_class=400)
            self.ab_to_l = TSN(num_segments=args.num_segments, num_class=400)
        # elif name == 'resnet18':
...
class TSMV2(nn.Module):
    def __init__(self, name='resnet50', args=None):
        super(TSMV2, self).__init__()
            # self.l_to_ab = TSN(num_segments=args.num_segments, num_class=args.feat_dim)
            # self.ab_to_l = TSN(num_segments=args.num_segments, num_class=args.feat_dim)
        # elif name == 'resnet18':
        #     self.l_to_ab = resnet18(in_channel=1, width=2)
...
class MyTSMCMC(nn.Module):
    def __init__(self, name='tsmv1', args=None):
        super(MyTSMCMC, self).__init__()
        if name.endswith('v1'):
            self.encoder = TSMV2(name[:-2], args)
