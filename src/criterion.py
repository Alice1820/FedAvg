...
class FixMatchLoss(torch.nn.CrossEntropyLoss):
    ...
    Args:
        ...
        threshold: A float in [0.0, 1.0], threshold for pseudo-labeling
        T: temperature for psedo-labeling
    ...
    def forward(self, input_w: Tensor, input_s: Tensor) -> Tensor:
        # Single modality
...
class MultiMatchLoss(torch.nn.CrossEntropyLoss):
    ...
    def forward(self, input_a_w: Tensor, input_a_s: Tensor, input_b_w: Tensor, input_b_s: Tensor) -> Tensor:
        ...
        max_probs_b, targets_b_u = torch.max(pseudo_label_b, dim=-1) # [bs, 1]
        mask_b = max_probs_b.ge(self.threshold).float() # prob of max_probs > threshold

        return F.cross_entropy(input_a_s, targets_a_u, reduction='mean') * mask_a + \
                F.cross_entropy(input_b_s, targets_b_u, reduction='mean') * mask_b + \
                F.cross_entropy(input_a_s, targets_b_u, reduction='mean') * mask_b + \
                F.cross_entropy(input_b_s, targets_a_u, reduction='mean') * mask_a